    ...
    def inference(results, model, xyz, z_vals, test_time=False, **kwargs):
        ...
        if typ=='coarse' and test_time:
            ...
            for i in range(0, B, chunk):
                ...
                out_chunks += [model(input_tensor, output_transient=output_transient)]
        ...
        if output_transient:
            ...
            transient_rgb_map = reduce(rearrange(transient_weights, 'n1 n2 -> n1 n2 1')*transient_rgbs,
                                       'n1 n2 c -> n1 c', 'sum')
            ...
            if test_time:
                static_alphas_shifted = torch.cat([torch.ones_like(static_alphas[:, :1]), 
                                                 1-static_alphas], -1)
                static_transmittance = torch.cumprod(static_alphas_shifted[:, :-1], -1)
                static_weights_ = static_alphas * static_transmittance
                static_rgb_map_ = reduce(rearrange(static_weights_, 'n1 n2 -> n1 n2 1')*static_rgbs,
                                       'n1 n2 c -> n1 c', 'sum')
                if white_back:
                ...
                transient_alphas_shifted = torch.cat([torch.ones_like(transient_alphas[:, :1]), 
                                                    1-transient_alphas], -1)
                ...
                results['rgb_fine_transient'] = reduce(rearrange(transient_weights_, 
                                                               'n1 n2 -> n1 n2 1')*transient_rgbs,
                                                     'n1 n2 c -> n1 c', 'sum')
                results['depth_fine_transient'] = reduce(transient_weights_*z_vals, 
                                                       'n1 n2 -> n1', 'sum')
        else:
